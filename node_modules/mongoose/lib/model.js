 ...
 * @event `index`: Emitted after `Model#createIndexeses` completes. If an error occurred it is passed with the event.
 * @event `index-single-start`: Emitted when an individual index starts within `Model#createIndexeses`. The fields and options being used to build the index are also passed with the event.
 * @event `index-single-done`: Emitted when an individual index finishes within `Model#createIndexeses`. If an error occurred it is passed with the event. The fields, options, and index name are also passed.
 * @api public
 */
...
Model.init = function init(callback) {
  ...
  const _createIndexeses = autoIndex ?
    cb => this.createIndexeses({ _automatic: true }, cb) :
    cb => cb();
 ...
 *
 * @param {Object} [options] options to pass to `createIndexeses()`
 ...
 * ####Example:
 *
 *     Event.createIndexeses(function (err) {
 *       if (err) return handleError(err);
 *     });
 *
...
Model.createIndexeses = function createIndexeses(options, callback) {
  _checkContext(this, 'createIndexeses');
  ...
  return promiseOrCallback(callback, cb => {
    ...
    _createIndexeses(this, options || {}, error => {
      if (error) {
        return cb(error);
      }
      cb(null);
...
/**
 * Similar to `createIndexeses()`, except for it uses the [`createIndex`](http://mongodb.github.io/node-mongodb-native/2.2/api/Collection.html#createIndex)
 * function.
 *
 * @param {Object} [options] internal options
 * @param {Function} [cb] optional callback
...
Model.createIndexes = function createIndexes(options, callback) {
  ...
  return this.createIndexeses(options, callback);
};
...
function _createIndexeses(model, options, callback) {
  ...
  const create = function() {
    ...
    const methodName = useCreateIndex ? 'createIndex' : 'createIndexes';
